<x-app-layout>
    ...
    <div class="flex-1 p-8-dashboard">
        ...
        @unless(isset($results))
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                @foreach ($stats as $stat)
                    <div
                        class="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl p-6 rounded-2xl shadow-xl border border-white/20 dark:border-gray-700/50 hover:shadow-2xl transition-all duration-300 transform hover:scale-105">
                        <div class="flex items-center justify-between">
                            ...
                            <div
                                class="w-16 h-16 bg-gradient-to-r {{ $stat['gradient'] }} rounded-2xl flex items-center justify-center shadow-lg">
                        ...
                        <div class="mt-4 bg-gradient-to-r {{ $stat['bg'] }} dark:{{ $stat['dark_bg'] }} rounded-xl p-3">
                            ...
                            <div class="mt-2 w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                                <div class="bg-gradient-to-r {{ $stat['gradient'] }} h-2 rounded-full transition-all duration-500"
                                    style="width: {{ $stat['count'] > 0 ? min(($stat['count'] / max(array_column($stats, 'count'))) * 100, 100) : 0 }}%">
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        @else
            ...
            @else
                <div
                    class="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl rounded-2xl shadow-xl border border-white/20 dark:border-gray-700/50 p-12 text-center">
                    <div
                        class="w-24 h-24 bg-gradient-to-r from-blue-100 to-purple-100 dark:from-blue-900/20 dark:to-purple-900/20 rounded-full flex items-center justify-center mx-auto mb-6">
            ...
            @if(!empty($recentActivity))
                <div
                    ...
                    <div class="flex items-center mb-6">
                        <div
                            class="w-12 h-12 bg-gradient-to-r from-amber-500 to-orange-600 rounded-xl flex items-center justify-center mr-4">
                    ...
                    <div class="space-y-4">
                        @foreach($recentActivity as $activity)
                            <div
                                class="flex items-start space-x-4 p-4 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-all duration-200">
        ...
        @foreach (['error' => 'red', 'success' => 'green', 'warning' => 'yellow', 'info' => 'blue'] as $type => $color)
            @if (session($type))
                <div class="fixed top-24 right-6 z-50">
                    <div class="bg-gradient-to-r from-{{ $color }}-500 to-{{ $color }}-600 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center transform transition-all duration-500 translate-x-full opacity-0"
                        id="session-{{ $type }}">
                        <i
                            class="fas fa-{{ $type === 'error' ? 'exclamation-circle' : ($type === 'success' ? 'check-circle' : ($type === 'warning' ? 'exclamation-triangle' : 'info-circle')) }} text-2xl mr-4"></i>
                        <div>
