<div class="p-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
  <div class="mb-8">
    <div class="flex items-center justify-between mb-4">
      ...
      <div class="flex items-center gap-3">
        ...
        <button onclick="toggleViewMode()" id="view-toggle"
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
      ...
      <div class="text-center">
        <div class="text-2xl font-bold text-yellow-600 dark:text-yellow-400">
          {{ $tasks->where('priorite', 'élevée')->count() }}
        </div>
        <div class="text-sm text-gray-600 dark:text-gray-400">Priorité élevée</div>
      </div>
      <div class="text-center">
        <div class="text-2xl font-bold text-green-600 dark:text-green-400">
          {{ $tasks->where('date_limite', '<', now())->count() }}
        </div>
        <div class="text-sm text-gray-600 dark:text-gray-400">En retard</div>
      </div>
      <div class="text-center">
        <div class="text-2xl font-bold text-purple-600 dark:text-purple-400">
          {{ $tasks->whereNull('assignes')->count() }}
        </div>
  ...
  <div id="tasks-container">
    <div id="card-view" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      ...
      <div
      class="task-card bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-all duration-200 cursor-pointer"
      data-task-id="{{ $task->id }}" data-status="{{ $task->colonne_id }}" data-priority="{{ $task->priorite }}"
      data-assignees="{{ $task->assignes->pluck('id')->implode(',') }}" onclick="openTaskModal({{ $task->id }})">

      <div class="p-4 border-b border-gray-100 dark:border-gray-700">
        <div class="flex items-start justify-between mb-2">
        <h3 class="font-semibold text-gray-900 dark:text-white text-sm line-clamp-2">{{ $task->titre }}</h3>
        <div class="flex items-center gap-1">
          @if($task->priorite === 'élevée')
      ...
      <span
      class="inline-block bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 px-2 py-1 rounded-full text-xs">
      {{ $tag->name }}
      </span>
      @endforeach
      </div>
    ...
    @endif
      ...
      <div class="p-4">
        ...
        <span
          class="px-2 py-1 text-xs font-medium rounded-full {{ $task->colonne->couleur ?? 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200' }}">
          {{ $task->colonne->nom }}
        </span>

        @if($task->date_limite)
      <span
        class="text-xs {{ $task->date_limite < now() ? 'text-red-500' : 'text-gray-500 dark:text-gray-400' }}">
      ...
      @empty
        <div
        class="w-6 h-6 bg-gray-200 dark:bg-gray-600 rounded-full border-2 border-white dark:border-gray-700 flex items-center justify-center">
      ...
      @endforelse
        ...
        <span
        class="w-6 h-6 bg-gray-200 dark:bg-gray-600 rounded-full border-2 border-white dark:border-gray-700 flex items-center justify-center text-xs">
      ...
      <div
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            ...
            data-task-id="{{ $task->id }}" data-status="{{ $task->colonne_id }}"
            data-priority="{{ $task->priorite }}" data-assignees="{{ $task->assignes->pluck('id')->implode(',') }}">
          ...
          @endif
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              <span
              class="px-2 py-1 text-xs font-semibold rounded-full {{ $task->colonne->couleur ?? 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200' }}">
              {{ $task->colonne->nom }}
              </span>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
          ...
          {{ $task->priorite === 'élevée' ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' : '' }}
          ...
          @endif
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              ...
              @forelse($task->assignes->take(3) as $assigne)
          <img src="{{ $assigne->avatar ?? '/default-avatar.png' }}" alt="{{ $assigne->name }}"
            class="w-8 h-8 rounded-full border-2 border-white dark:border-gray-700"
            title="{{ $assigne->name }}">
          @empty
